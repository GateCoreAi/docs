...
<Info>
    To access state, add `runtime: ToolRuntime` to your tool signature. When the tool runs, the runtime (for example, [`ToolNode`](https://reference.langchain.com/python/langgraph/agents/#langgraph.prebuilt.tool_node.ToolNode)) supplies the value, not the model, and the parameter is omitted from the tool schema sent to the model.
</Info>
