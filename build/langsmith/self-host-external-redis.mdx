...
* **Supported versions:** Redis >= 6.2, or Valkey 8. Valkey is treated as a drop-in replacement for Redis throughout this guide.
  ...
  <Note>
    Redis 6.2 or later is required. As of `langgraph-api` 0.8.0 (April 2026), the Redis-backed run queue uses a `ZADD` flag introduced in Redis 6.2. Running Redis 5.x–6.1.x will cause a generic `ERR` syntax error on run enqueue that does not clearly indicate a version mismatch. Upgrade to Redis 6.2+ before installing or upgrading to LangSmith with Agent Server 0.8.0 or later.
  </Note>
