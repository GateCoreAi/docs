...
Once a role is restricted, the member invite and member edit flows—and the API-key creation flow—will hide it from users who do not hold the Workspace Admin role, and the API will reject attempts to assign it from non-Workspace-Admin callers. This also covers the fallback case: a creator who does not hold the Workspace Admin role cannot create a key that defaults to their own role when that role is restricted.
