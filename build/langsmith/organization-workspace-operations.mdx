...
<Note>
\* Organization Operators and Organization Users can create and delete workspace-scoped service keys only for workspaces where they can manage keys (a role granting `workspaces:manage-keys`, which the Workspace Admin role includes). Org-wide service keys require the Organization Admin role.
</Note>
