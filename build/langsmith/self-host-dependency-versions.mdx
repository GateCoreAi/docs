...
| [Redis](/langsmith/self-host-external-redis) | 6.2 | Used for queueing and caching. Standalone and Redis Cluster modes are both supported. As of Agent Server 0.8.0, the Redis-backed run queue requires Redis 6.2+ (uses a `ZADD` flag introduced in 6.2). |
