...
- **Self-hosted only — Redis version requirement raised to 6.2+:** The Redis-backed run queue uses a `ZADD` flag introduced in Redis 6.2. **If you are running Redis 5.x or 6.0–6.1, upgrading to Agent Server 0.8.0 will cause run enqueue to fail with a generic `ERR` syntax error** that does not clearly indicate a version mismatch. Upgrade your Redis instance to 6.2 or later before deploying this version. Valkey 8 is also supported. See [self-hosted dependency versions](/langsmith/self-host-dependency-versions) for the full requirements.
</Update>
