...
:::js
TypeScript does not support the convenience environment variables. Configure `ChatOpenAI` with the gateway URL and your LangSmith API key directly:
...
<Note>
Use a `provider/model-name` model slug when you configure the gateway directly.
</Note>
:::
